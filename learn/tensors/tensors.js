    ...
    render({
        ...
        section: {
            ...
            dialog: [
                ...
                message("W", "... and all elements in its first elements are `int`s, all elements in its second elements are `int`s."),
                ...
                message("D", "Yes, `Tensors` can be nested. Actually, `Tensor[int][[2, 3]]` is aleady nested."),
                message("W", "So, `Tensor[int][[2, 3]]` and `Tensor[Tensor[int][[3]]][[2]]` must be identical!"),
                message("D", "Right. We say that they are the same type.\nNow, how about `Tensor[int][[]]`? Is it identical to another type?"),
                message("W", "Are `Tensor[int][[]]` and `int` the same type?"),
                message("D", "They are. `42` is an `int`, as well as a `Tensor[int][[]]`. We call such values scalars."),
            ],
        },
    });
})();
