(() => {
    ...
    render({
        ...
        dialog: [
            ...
            message("D", "`Num` is a type that describes the behavior shared by all `int` values and all `float` values."),
            ...
            message("D", "Fortunately, we have machines to do that reasoning for us, rigorously and efficiently.\n" +
            ...
            message("D", "It has the type `Tensor[int][[2, 3]]`. In addition to the element type, we set a constraint on the element shape."),
            message("W", "Oh! `[2, 3]` is a `List` that describes the shape of `[[1, 2, 3], [4, 5, 6]]`, since the outer layer contains two `List[int]`s and each inner layer contains three `int`s?"),
            ...
            message("D", "That's right. `42` is an `int`, and also a `Tensor[int][[]]`. When a `Tensor` has shape `[]`, we call it a scalar."),
            ...
            message("W", "Such as?"),
            message("D", "Running programs on GPUs, in parallel and efficiently!"),
