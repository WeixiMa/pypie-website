    ...
    render({
        ...
        dialog: [
            message("D", "Now let's run programs with `Tensor`s.\n" +
                "What is `1 + 1`?"),
            message("W", "`2`. That's easy and they are not even `Tensor`s!"),
            message("D", "`2` is correct, but [it wasn't too easy.](https://en.wikipedia.org/wiki/Principia_Mathematica#/media/File:Principia_Mathematica_54-43.png)\n" +
                "Also, `1` is actually a `Tensor[int][[]]`.\n" +
                "Here's a more interesting one: `Tensor([1]) + Tensor([1])`."),
            message("W", "Ok, perhaps we may add elements at matching positions. " +
                "It's `Tensor([2])`, right?\n" +
            ...
            message("D", "Yes, as long as the smaller shape is a tail of the larger shape.\n" +
            ...
            message("W", "Ooph! What an intimidating word!"),
            message("D", "It is actually warmer and fuzzier than it sounds. Here, **rank** tells us the size of the shape.\n" +
                "For example, " +
                "`Tensor[int][[3]]` is rank `1` and `Tensor[int][[2, 3]]` is rank `2`."),
            message("W", "So, `42` has type `Tensor[int][[]]`, which makes it rank `0`?"),
            message("D", "Correct! When designing a function, we need to define the shapes for its inputs and output. " +
            ...
            message("W", "Now it's less intimidating. You mentioned functions, is `+` a **function**?"),
            message("D", "`+` is a function. Just like\n" +
                "`5` has type `Tensor[int][[]]`,\n" +
                "`+` has type `(x: Tensor[int][[]], y: Tensor[int][[]]) -> Tensor[int][[]]`."),
            message("W", "Let me guess. This function type tells us:\n" +
            ...
            message("D", "Let's try the more difficult one!\n`Tensor([[1, 2, 3], [3, 2, 1]]) + Tensor([5, 7, 9])`."),
            message("W", "Now `x` has type `Tensor[int][[2, 3]]` and `y` has type `Tensor[int][[3]]`. " +
                "Neither precisely matches the expected `Tensor[int][[]]`... now what?"),
            message("D", "Rank polymophism comes to rescue. It works in two steps, both rely on our **tail** trick."),
            message("W", "Please show me the tail trick!"),
            message("D", "The trick is to find the **tail** and the **lead**. When comparing two shapes of different ranks, " +
            ...
            message("D", "Now we use the trick, twice.\n" +
            ...
            message("W", "Please let me try the first step!\n" +
            ...
            message("D", "We now have the leads! Go for the lead of leads."),
            message("W", "Between `[2, 3]` and `[3]`, the lead is `[2]`."),
            message("D", "Great! By finding the lead of leads, we have showned that `+` may apply to a `Tensor[int][[2, 3]]]` and a `Tensor[int][[3]]`.\n" +
                "We are not done yet--the function also has an output type."),
            message("W", "In `+`'s type, its output is `Tensor[int][[]]`. Is it changed by rank polymorphism?"),
            message("D", "We are applying `+` on the tail, repeatedly for the leading positions. Our output should be a repeated `Tensor[int][[]]`."),
            message("W", "So, we just need to find out the number of repetitions--the lead that contributes to the lead of the leads!"),
            message("D", "Mouthful, but a good observation. In our case, it is ...?"),
            message("W", "`[2, 3]`. The result of `Tensor([[1, 2, 3], [3, 2, 1]]) + Tensor([5, 7, 9])` has type `Tensor[Tensor[int][[]]][[2, 3]]`!\n" +
                "That is repeating `Tensor[int][[]]`, for `[2, 3]` times.\n" +
                "And machines should be able to simplify it to the equivalent `Tensor[int][[2, 3]]`."),
            message("D", "Congratz on mastering rank polymorphism!\n" +
            ...
            message("D", "To learn how and why it works, we walked through the reasoning step by step manually. " +
