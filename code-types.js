...
class DocBuilder {
    ...
    currentLineLength() {
        ...
        const line = this.lines[this.lines.length - 1];
        return line.reduce((total, segment) => total + String(segment?.text ?? "").length, 0);
    }
    toHtml() {
...
const estimateTypeLength = (node) => {
    ...
    if (node.kind === "TypeList") {
        const itemsLength = (node.items || []).reduce((sum, item, index) => sum + estimateTypeLength(item) + (index < node.items.length - 1 ? 2 : 0), 0);
        return 1 + itemsLength + 1;
    }
    if (node.kind === "TypeSubscript") {
        const indexLength = node.index?.kind === "TypeList"
            ? (node.index.items || []).reduce((sum, item, index) => sum +
                estimateTypeLength(item) +
                (index < node.index.items.length - 1 ? 2 : 0), 0)
...
const estimateExprLength = (node) => {
    ...
    switch (node.kind) {
        ...
        case "Call": {
            const argsLength = (node.args || []).reduce((sum, argNode, index) => sum + estimateExprLength(argNode) + (index < node.args.length - 1 ? 2 : 0), 0);
        ...
        case "List": {
            const itemsLength = (node.elements || []).reduce((sum, element, index) => sum + estimateExprLength(element) + (index < node.elements.length - 1 ? 2 : 0), 0);
            return 1 + itemsLength + 1;
        }
        case "Tuple": {
            const itemsLength = (node.elements || []).reduce((sum, element, index) => sum + estimateExprLength(element) + (index < node.elements.length - 1 ? 2 : 0), 0);
        ...
        case "ListComp": {
            const targetLength = (node.target || []).reduce((sum, targetNode, index) => sum + estimateExprLength(targetNode) + (index < node.target.length - 1 ? 2 : 0), 0);
            return (1 +
        ...
        case "BinOp":
            return (estimateExprLength(node.left) +
                node.op.length +
                2 +
                estimateExprLength(node.right));
        case "BoolOp": {
            const sep = node.op.length + 2;
            return (node.values || []).reduce((sum, valueNode, index) => sum + estimateExprLength(valueNode) + (index < node.values.length - 1 ? sep : 0), 0);
        }
        case "Compare":
            return (estimateExprLength(node.left) +
                node.op.length +
                ...
                estimateExprLength(node.right));
        case "IfExpr":
            return (estimateExprLength(node.body) +
                4 +
...
const estimateArgLength = (argNode) => estimateIdentifierLength(argNode?.name) + 2 + estimateTypeLength(argNode?.annotation);
const wouldOverflow = (builder, trailingLength) => builder.currentLineLength() + trailingLength > MAX_LINE_LENGTH;
...
const printExpr = (builder, node, parentPrecedence = 0) => {
    ...
    switch (node.kind) {
        ...
        case "Call": {
            ...
            builder.token("(");
            const inlineArgsLength = (node.args || []).reduce((sum, argNode, index) => sum + estimateExprLength(argNode) + (index < node.args.length - 1 ? 2 : 0), 0);
            const shouldWrapArgs = node.args.length > 0 && wouldOverflow(builder, inlineArgsLength + 1);
            if (shouldWrapArgs) {
                builder.newline();
                builder.indent();
        ...
        case "List": {
            builder.token("[");
            const inlineElementsLength = (node.elements || []).reduce((sum, element, index) => sum + estimateExprLength(element) + (index < node.elements.length - 1 ? 2 : 0), 0);
            const shouldWrapElements = node.elements.length > 0 && wouldOverflow(builder, inlineElementsLength + 1);
        ...
        case "Tuple": {
            ...
            const inlineElementsLength = (node.elements || []).reduce((sum, element, index) => sum + estimateExprLength(element) + (index < node.elements.length - 1 ? 2 : 0), 0);
            const trailingComma = node.elements.length === 1 ? 1 : 0;
            const shouldWrapElements = node.elements.length > 0 &&
        ...
        case "ListComp": {
            ...
            const inlineCompLength = estimateExprLength(node.elt) +
                5 +
                (node.target || []).reduce((sum, targetNode, index) => sum + estimateExprLength(targetNode) + (index < node.target.length - 1 ? 2 : 0), 0) +
                4 +
                estimateExprLength(node.iter) +
                1;
            const shouldWrapComp = wouldOverflow(builder, inlineCompLength) || node.elt?.kind === "IfExpr";
            if (shouldWrapComp) {
                builder.newline();
                builder.indent();
...
const printStatement = (builder, node) => {
    switch (node.kind) {
        ...
        case "FunctionDef":
            ...
            printIdentifier(builder, node.name);
            builder.token("(");
            const inlineArgsLength = (node.args || []).reduce((sum, argNode, index) => sum + estimateArgLength(argNode) + (index < node.args.length - 1 ? 2 : 0), 0);
            const returnSuffixLength = 1 + 4 + estimateTypeLength(node.returns) + 1;
            const shouldWrapArgs = node.args.length > 0 &&
